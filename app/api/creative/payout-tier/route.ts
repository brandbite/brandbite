// -----------------------------------------------------------------------------
// @file: app/api/creative/payout-tier/route.ts
// @purpose: Creative API — current payout tier and progress toward each rule
// @version: v1.0.0
// @status: active
...
import { BASE_PAYOUT_PERCENT, evaluateCreativePayoutPercent } from "@/lib/token-engine";
  ...
  try {
    ...
    if (user.role !== "DESIGNER") {
      return NextResponse.json(
        { error: "Only creatives can access payout tier info" },
        { status: 403 },
      );
    }

    // Get current tier
    const evaluation = await evaluateCreativePayoutPercent(user.id);
    ...
    console.error("[creative.payout-tier] GET error", error);
