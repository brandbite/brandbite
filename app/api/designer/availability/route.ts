// -----------------------------------------------------------------------------
// @file: app/api/designer/availability/route.ts
// @purpose: Designer pause/resume API for controlling assignment availability
// @version: v1.0.0
// @status: active
// -----------------------------------------------------------------------------
...
import {
  ...
  formatPauseStatus,
} from "@/lib/designer-availability";
  ...
  try {
    ...
    if (user.role !== "DESIGNER") {
      return NextResponse.json(
        { error: "Only designers can access availability" },
        { status: 403 },
      );
    }

    const [designer, activeTicketCount] = await Promise.all([
    ...
    const effectivelyPaused = isDesignerPaused(designer);
    if (designer.isPaused && !effectivelyPaused) {
    ...
    console.error("[designer.availability] GET error", error);
    return NextResponse.json(
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (user.role !== "DESIGNER") {
      return NextResponse.json(
        { error: "Only designers can manage availability" },
        { status: 403 },
      );
    }
    ...
    console.error("[designer.availability] PATCH error", error);
    return NextResponse.json(
