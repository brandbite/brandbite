...
export async function POST(req: NextRequest) {
  try {
    ...
    const ticketId =
      typeof raw.ticketId === "string" && raw.ticketId.length > 0 ? raw.ticketId : null;
    ...
    if (ticketId) {
      ...
      if (!ticket) {
        return NextResponse.json({ error: "Ticket not found for this company" }, { status: 400 });
      }
    }
