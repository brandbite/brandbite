...
export default function CreativeTicketDetailPage() {
  ...
  useEffect(() => {
    ...
    if (!resolvedTicketId) {
      console.warn(
        "[CreativeTicketDetailPage] Could not resolve ticket id from URL",
      );
      return;
    }
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch(
          `/api/creative/tickets/${resolvedTicketId}`,
          {
            cache: "no-store",
          },
        );
  ...
  useEffect(() => {
    ...
    const loadComments = async () => {
      ...
      try {
        const res = await fetch(
          `/api/creative/tickets/${ticketIdFromData}/comments`,
          {
            cache: "no-store",
          },
        );
  ...
  useEffect(() => {
    ...
    const loadRevisions = async () => {
      ...
      try {
        const res = await fetch(
          `/api/creative/tickets/${ticketIdFromData}/revisions`,
          {
            cache: "no-store",
          },
        );
        ...
        console.error(
          "Creative ticket revisions fetch error:",
          err,
        );
        if (!cancelled) {
          setRevisionsError(
        ...
        <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
          <div>
            <Button
              ...
              onClick={() =>
                router.push(
                  fromBoard ? "/creative/board" : "/creative/tickets",
                )
              }
              className="mb-2"
            >
              ...
              <div className="rounded-2xl border border-[var(--bb-border)] bg-white px-4 py-3 shadow-sm">
                ...
                <div className="mt-2 space-y-2 text-xs text-[var(--bb-text-secondary)]">
                  ...
                  <div>
                    ...
                    <p>
                      {ticket.creative?.name ||
                        ticket.creative?.email ||
                        "—"}
                    </p>
                  </div>
                </div>
