...
// @purpose: Designer-facing kanban board for assigned tickets with revision
...
export default function DesignerBoardPage() {
  ...
  // Designer note modal state for IN_PROGRESS -> IN_REVIEW transition
  ...
  useEffect(() => {
    ...
    const loadBriefAssets = async () => {
      ...
      try {
        const res = await fetch(
          `/api/designer/tickets/${detailTicketId}/assets?kind=BRIEF_INPUT`,
          { cache: "no-store" },
        );
        const json = await res.json().catch(() => null);
  ...
  const handleBulkStatusChange = async (targetStatus: string) => {
    ...
    try {
      const res = await fetch("/api/designer/tickets/batch", {
        method: "PATCH",
      ...
      console.error("[DesignerBoard] bulk update error", err);
      showToast({
        type: "error",
        title: "Bulk update failed",
  ...
  const handleColumnDrop = async (
    ...
    // IN_PROGRESS -> IN_REVIEW: open modal first, collect designer note
  ...
  return (
    <>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-[10px] text-[#9a9892]">
                      Your active designer workspace.
                    </p>
                  </div>
                  {stats && (
                      ...
                      <div className="space-y-5">
                        ...
                        <div>
                          ...
                          <div className={`rounded-xl border-2 bg-[#fbfaf8] px-4 py-4 ${
                            ...
                            {typeof latestRev.designerMessage === "string" &&
                              latestRev.designerMessage.trim().length > 0 && (
                  ...
                  <div className="space-y-3 text-xs">
                    ...
                    {detailTicket.jobType && (
                      <div>
                        ...
                        <p className="mt-0.5 font-semibold text-[#424143]">
                          {detailTicket.jobType.designerPayoutTokens} tokens
                        </p>
                      </div>
                    )}
                  </div>
      ...
      {/* Upload work modal (file upload + designer note) */}
