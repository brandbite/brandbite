...
_Last updated: 2026-04-23 — Security Plan L1–L6 shipped (audit log, confirmations, receipts, blocked alerts, email MFA + TOTP upgrade, 2-person approval) + auth hardening pass (per-email rate limit, password complexity, session revocation on reset, explicit cookie attributes) + defense-in-depth pass (dompurify CVE fix, CSP form-action / frame-ancestors / upgrade-insecure-requests, COOP, X-Permitted-Cross-Domain-Policies) + Stripe webhook idempotency (reserve/release ledger, prevents double-credit on retry)_
...
- **Stripe webhook idempotency** (2026-04-23 PR): added a `ProcessedStripeEvent` dedup ledger with `eventId` as primary key + helper pair in `lib/stripe-webhook-idempotency.ts` (`reserveStripeEvent` / `releaseStripeEvent`). The webhook handler at `app/api/billing/webhook/route.ts` now reserves the event after signature verification; a replay (Stripe retries any non-2xx for up to 3 days) returns 200 immediately without running the handler body. Handler failures release the reservation so Stripe's retry can reprocess cleanly. Closes a real latent bug: `invoice.payment_succeeded` previously credited `plan.monthlyTokens` every delivery — a retry (from our 500, or a Stripe-side response loss) would double-credit monthly tokens into a customer's ledger. 6 integration tests covering the reserve / replay / different-id / release / missing-row / DB-level-constraint paths.
- **Defense-in-depth pass** (2026-04-23 PR): pre-launch sweep of HTTP-level + dependency hardening. **`npm audit fix` on runtime deps** cleared a batch of known CVEs — most importantly **dompurify 3.3.x → 3.4.1** which patched several XSS-bypass advisories (mutation-XSS, `FORBID_TAGS` bypass, prototype pollution via `CUSTOM_ELEMENT_HANDLING`). Since we rely on DOMPurify for CMS HTML sanitization on `/blog`, `/news`, `/showcase`, and `/docs`, this upgrade directly strengthens A1 (stored-XSS prevention). Also bumped `defu` (Prisma config prototype pollution), `effect` (async-context contamination), and `brace-expansion` (DoS). Two remaining transitive moderates (`fast-xml-parser` in AWS SDK v3, `uuid` in Resend/svix) would require breaking `--force` major-bumps; neither exploit path is reachable from our usage. **CSP tightened** with `form-action 'self'` (blocks phishing-via-form-exfil), `frame-ancestors 'none'` (CSP-native clickjacking protection, modern replacement for X-Frame-Options), `upgrade-insecure-requests` (auto http→https belt-and-braces with HSTS), and `worker-src 'self' blob:`. **Two new response headers**: `Cross-Origin-Opener-Policy: same-origin` (Spectre / `window.opener` isolation) and `X-Permitted-Cross-Domain-Policies: none` (obsolete Flash/PDF exfil vector, zero-cost).
