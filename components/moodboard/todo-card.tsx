  ...
  return (
    <div className="p-4">
      ...
      {editingTitle ? (
        ...
        <div
          className="mb-2 cursor-pointer min-h-[1.25rem]"
          onClick={() => {
            setTitle(data.title ?? "");
        ...
        {data.items.map((item) => (
          <li key={item.id} className="flex items-center gap-2">
            <input
              ...
              className="w-4 h-4 rounded accent-[var(--bb-primary)] flex-shrink-0 cursor-pointer"
            />
            ...
            {editingItemId === item.id ? (
              <input
                ...
                className="flex-1 text-sm text-[var(--bb-secondary)] bg-transparent border-none outline-none placeholder:text-gray-400"
              />
            ) : (
              <span
                className={`flex-1 text-sm cursor-pointer ${
                  item.checked
                    ? "line-through text-[var(--bb-text-secondary)]"
                    : "text-[var(--bb-secondary)]"
                }`}
                onClick={() => startEditItem(item)}
              >
                {item.text || (
                  <span className="text-gray-400 italic">Empty item</span>
                )}
              </span>
            )}
          </li>
      ...
      <div className="flex items-center justify-between mt-3 pt-2 border-t border-[var(--bb-border)]">
        <button
          onClick={addItem}
