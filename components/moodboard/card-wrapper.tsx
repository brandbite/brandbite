...
export function CardWrapper({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group relative rounded-2xl border border-[var(--bb-border)] bg-white shadow-sm overflow-hidden transition-shadow hover:shadow-md ${
      ...
      <div className="absolute top-2 right-2 z-10 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div ref={menuRef} className="relative">
          <button
            ...
            className="flex items-center justify-center w-7 h-7 rounded-lg hover:bg-gray-100"
          ...
          {menuOpen && (
            <div className="absolute right-0 top-8 w-40 rounded-xl border border-[var(--bb-border)] bg-white shadow-lg py-1 z-20">
              {onToggleWidth && (
                <button
                  onClick={() => {
                    onToggleWidth();
                  ...
                  className="w-full text-left px-3 py-2 text-sm text-[var(--bb-secondary)] hover:bg-gray-50"
                >
                  Toggle width
                </button>
              )}
