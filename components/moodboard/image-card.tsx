      ...
      <div className="p-3">
        {editingCaption ? (
          <input
            ...
            className="w-full text-sm text-[var(--bb-secondary)] bg-transparent border-none outline-none placeholder:text-gray-400"
          />
        ) : (
          <p
            className="text-sm text-[var(--bb-text-secondary)] cursor-pointer min-h-[1.25rem]"
            onClick={() => setEditingCaption(true)}
          >
            {data.caption || (
              <span className="text-gray-400 italic">Add a caption...</span>
            )}
          </p>
        )}
      </div>
