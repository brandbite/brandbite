...
export const auth = betterAuth({
  ...
  emailVerification: {
    // Note: BetterAuth's post-verify redirect is NOT controlled by an
    // `emailVerification.callbackURL` config option (PR #215 mistakenly
    // added one and it had no effect). The library reads `callbackURL`
    // from the sign-up request BODY at the moment the verification email
    // is built — see app/login/page.tsx where we pass it on the
    // `authClient.signUp.email({ ..., callbackURL: "/onboarding" })`
    // call. The resend endpoint similarly takes `callbackURL` in its body
    // (handled in app/verify-email/page.tsx).
    //
